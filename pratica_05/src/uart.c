      ...
      switch(uart){
         case(UART0): // tx=1.11  rx=1.10  cts=1.8  rts=1.9
            ...
            padSetMode(CM_conf_uart0_rxd,MODE_0); // set p1.10 as UART0_rx
						...
						unsigned int temp = ckmGetCLKModuleRegister(CKM_WKUP, CKM_WKUP_CLKSTCTRL);
            temp &= ~(0b11);
            temp |= 0b10;      // software-forced wake-up transition on the "always on clock domain", TRM Table 8-92
						ckmSetCLKModuleRegister(CKM_WKUP, CKM_WKUP_CLKSTCTRL,temp);
           
            temp = ckmGetCLKModuleRegister(CKM_PER, CKM_PER_L4HS_CLKSTCTRL);
	          temp &= ~(0b11);
            temp |= 0b10;      // software-forced wake up transition on the L4 high speed domain
            ckmSetCLKModuleRegister(CKM_PER, CKM_PER_L4HS_CLKSTCTRL,temp);
            
						temp = ckmGetCLKModuleRegister(CKM_WKUP, CKM_WKUP_UART0_CLKCTRL);
	        	temp &= ~(0b11);
            temp |= 0b10;      // Module is explicitly enabled,    TRM Table 8-137
            ckmSetCLKModuleRegister(CKM_WKUP, CKM_WKUP_UART0_CLKCTRL,temp);
					  while((ckmGetCLKModuleRegister(CKM_WKUP, CKM_WKUP_UART0_CLKCTRL) & (0b11<<16)) != 0); //wait until clock transition is complete
            ...
            temp = ckmGetCLKModuleRegister(CKM_PER, CKM_PER_UART1_CLKCTRL);
            temp &= ~(0b11);
            temp |= 0b10;      // Module is explicitly enabled,    TRM Table 8-137
            ckmSetCLKModuleRegister(CKM_PER, CKM_PER_UART1_CLKCTRL, temp);
